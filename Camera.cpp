...
void Camera::update(float zoom, bool dragging, bool strafing) {
	glfwGetCursorPos(_window, &x, &y);
	int b = static_cast<int>(x - _mouseX);
	int e = static_cast<int>(y - _mouseY);
	int f = static_cast<int>(x + _mouseX);
	int g = static_cast<int>(y + _mouseX);
	float speed = 0.005f;
	glm::vec3 i;
	if (dragging) {
		ggg += b * speed;
